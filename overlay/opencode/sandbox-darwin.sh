...
setup_sandbox() {
  ...
  # isolated_home が symlink する前提のため先に作る (attach サーバー登録の置き場)
  mkdir -p "${REAL_HOME}/.local/state/opencode"
  isolated_home
  gpg_agent
  container_socket
...
launch_herdr_context() {
  ...
  if [[ -n $pane_id ]]; then
    ...
    created=$(hcli workspace create --cwd "$PROJECT_DIR" --label "$new_label" --focus)
