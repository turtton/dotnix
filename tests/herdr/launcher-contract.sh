#!/usr/bin/env bash
# Usage: launcher-contract.sh <launcher-script-or-binary>
# Contract for the herdr-context opencode launcher:
...
expected_label() {
  local dir=$1
  printf 'opencode-%s' "$(basename "$dir" | LC_ALL=C sed 's/[^[:alnum:]]/-/g')"
}
